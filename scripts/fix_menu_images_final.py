...
def main():
    # 1. Parse old images from menu_head.ts
    with open('menu_head.ts', 'r', encoding='utf-16') as f:
        head_content = f.read()

    old_images = {}
    for match in re.finditer(r"id:\s*'([^']+)'.*?imageUrl:\s*'([^']*)'", head_content, re.DOTALL):
        # ensure we only match within the same block by checking if there's no "id:" in between
        block = match.group(0)
        if block.count("id: '") == 1:
            old_images[match.group(1)] = match.group(2)

    # 2. Custom images for new items
    custom_images = {
    ...
    # Combine them
    all_images = {**old_images, **custom_images}
    ...
    def replacer(match):
        ...
        best_img = ''
        if item_id in all_images and all_images[item_id]:
            best_img = all_images[item_id]
    ...
    new_content = re.sub(r"(id:\s*'([^']+)'.*?imageUrl:\s*'[^']*')", replacer, current_content, flags=re.DOTALL)
