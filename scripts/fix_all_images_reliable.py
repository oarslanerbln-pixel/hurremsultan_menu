...
unsplash_urls = {
    # Shishas
    's1': 'https://images.unsplash.com/photo-1510591509098-f4fdc6d0ff04?auto=format&fit=crop&w=600&q=80',
    ...
    'k1': 'https://images.unsplash.com/photo-1555562095-2c35e982d6df?auto=format&fit=crop&w=600&q=80',
    'k2': 'https://images.unsplash.com/photo-1581006852262-e4307cf6283a?auto=format&fit=crop&w=600&q=80',
    'k3': 'https://images.unsplash.com/photo-1596803244618-8dbee441d70b?auto=format&fit=crop&w=600&q=80',
}
...
def main():
    ...
    for i, line in enumerate(lines):
        ...
        if current_id and "imageUrl:" in line:
            ...
            # Perform exact string replacement to ensure we don't break TS syntax
            # The line might be: imageUrl: '',
            # We replace whatever is between the quotes
            lines[i] = re.sub(r"imageUrl:\s*'[^']*'", "imageUrl: '" + best_img + "'", line)
            # handle double quotes as well
            lines[i] = re.sub(r'imageUrl:\s*"[^"]*"', "imageUrl: '" + best_img + "'", lines[i])

    with open('../src/data/menu.ts', 'w', encoding='utf-8') as f:
        f.writelines(lines)
    
    print("All image URLs have been forcefully applied!")
